...
export function AdminPage(){
    ...
    async function getYouTubeVideoId(){
        try {
            ...
            if (!response.ok) throw new Error("Unable to get YouTube video ID");
    ...
    async function getProductData(){
        try {
            ...
            setProducts(data.result as DataInterface[]);
        } catch (err) {
            console.error(err);
    ...
    async function handleSubmit(e: React.FormEvent){
        ...
        const skipFields = ["productImages", "productName", "productDescription"];
        for (const key in formInputData) {
            if (skipFields.includes(key)) continue;
            const value = formInputData[key as keyof typeof formInputData];
    ...
    const handleFormTextChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
