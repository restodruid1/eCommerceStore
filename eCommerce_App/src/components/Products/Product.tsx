...
export function Product ({ data, pageUrl }:ProductProps) {
    ...
    return (
            ...
            <Link to={`/${pageUrl}/${data.id}/${data.name}`}><img src={`${data.url}`} style={{width:"100px", height:"200px", flex:"auto",margin: "5%", cursor:"pointer", backgroundColor:"lightgray", maxWidth:"100px"}}/></Link>
            <ol>
                <li>{data.id}</li> 
                <li>{data.name}</li> 
                <li>{data.quantity}</li> 
